...
export function BarcodePreview({ config, frame }: BarcodePreviewProps) {
  ...
  const createHighResCanvas = () => {
    ...
    const model = generateBarcodeModel(highResConfig);
    const sourceCanvas = document.createElement("canvas");
    renderBarcodeToCanvas(sourceCanvas, model, highResConfig);
  ...
  const handleDownloadPNG = () => {
    ...
    if (hasFrame && frame) {
      ...
      exportCanvasAsPNG(exportCanvas, "barcode.png");
      return;
    }
    exportCanvasAsPNG(canvasRef.current, "barcode.png");
  };

  const handleDownloadSVG = () => {
    if (!modelRef.current) return;
    const svg = exportBarcodeAsSVG(modelRef.current, config);
    downloadSVG(hasFrame && frame ? exportFramedSvg(svg, frame, modelRef.current.width, modelRef.current.height) : svg, "barcode.svg");
  };

  const handleDownloadPDF = () => {
    const exportCanvas = createHighResCanvas();
    exportCanvasAsPDF(exportCanvas, "barcode.pdf");
  };
            ...
            <div className="flex flex-col items-center gap-2.5">
              ...
              >
                ...
                <canvas
                  ...
                  style={{ maxWidth: "440px", maxHeight: "280px" }}
                />
              </div>
